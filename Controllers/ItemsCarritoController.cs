...
public class ItemsCarritoController : ControllerBase
{
    ...
    [HttpPost] public async Task<IActionResult> Post([FromBody] ItemCarritoCreateDTO dto)
    {
        if (!ModelState.IsValid) return BadRequest(ModelState);
        try { return Ok(await _service.CreateAsync(dto)); }
        catch (Exception ex) { return BadRequest(new { message = ex.Message }); }
    }
    [HttpPut("{id}")] public async Task<IActionResult> Put(int id, [FromBody] ItemCarritoUpdateDTO dto)
    {
        if (!ModelState.IsValid) return BadRequest(ModelState);
